...
export const AboutUsSection = () => {
  ...
  return (
    <section
      ref={ref}
      className="relative overflow-hidden border-t border-zinc-400/25 pt-12 pb-8 md:pt-16 md:pb-10 lg:pt-20 lg:pb-12"
    ...
    >
      ...
      {/* В devtools Chrome иногда рисует “полосу” из-за blur+opacity при анимации.
          Чтобы не показывать blur-слои в момент, когда они ещё прозрачные, монтируем их только после попадания в viewport. */}
      {isInView && (
        <>
          <motion.div
      ...
      {/* Мобилка: «труба» тоже стартует сверху секции (как на планшете/десктопе) */}
      <motion.div
        className="pointer-events-none absolute inset-x-0 top-0 z-[5] justify-center px-3 sm:px-4 md:hidden"
        initial={{opacity: 0, scale: 0.97}}
        animate={isInView ? {opacity: 1, scale: 1} : {opacity: 0, scale: 0.97}}
        transition={{duration: 1.25, ease, delay: 0.35}}
        aria-hidden
      >
        <div className="flex w-full max-w-[1200px] items-start gap-x-2 sm:gap-x-3">
          <div className="min-w-0 flex-[0.95]" />
          <div className="relative min-h-[min(66vw,380px)] w-full min-w-0 flex-[1.05] translate-x-1 sm:translate-x-2">
            <Image
              src="/images/about-us.png"
              alt=""
              fill
              sizes="(max-width: 767px) 52vw, 100vw"
      ...
      >
        <div className="flex w-full max-w-[1200px] items-start gap-x-2 sm:gap-x-4 md:gap-x-8 lg:gap-x-10 xl:gap-x-14">
          ...
          <div className="relative min-h-[clamp(120px,42vw,540px)] w-full min-w-0 flex-[1.12] md:min-h-[min(70vw,650px)] md:flex-[1.22] lg:min-h-[min(62vw,610px)]">
            <Image
              src="/images/about-us.png"
              alt=""
              fill
      ...
      <div className="relative z-10 mx-auto max-w-[1200px] px-3 sm:px-4">
        ...
        <div className="grid min-w-0 grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] items-start gap-x-2 gap-y-0 sm:gap-x-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1.22fr)] md:gap-x-8 lg:gap-x-10 xl:gap-x-14">
          <motion.div
          ...
          >
            <div className="space-y-2 rounded-lg border-l-2 border-zinc-500/35 pl-2 pt-0.5 sm:space-y-3 sm:pl-4 md:space-y-5 md:pl-8">
              ...
              <p className="hyphens-auto text-pretty text-left text-[#3d3d3d] md:text-justify font-['Open_Sans',_Helvetica,_Arial,_sans-serif] text-[clamp(8.5px,2.15vw,14px)] font-normal leading-[1.52] md:text-[clamp(10px,2.65vw,16px)] md:leading-[1.72]">
                ...
                постоянно работаем над развитием модельного ряда, в этом нам помогают клиенты,
                которые, не найдя подходящей модели, обращаются к нам с идеей, а мы воплощаем
          ...
          <div
            className="relative min-h-[min(66vw,380px)] w-full min-w-0 sm:min-h-[min(62vw,340px)] md:hidden"
            aria-hidden
          />
